...
const interval = setInterval(() => {
  value += increment;
  if (hpWidth> 10){hpWidth -= 10;}
  if (hpWidth === 10){
    //alert("게임오버")
    //gameOver = true;
    window.location.href = "file:///D:/지역연계/지역연계/html/fail.html";
  }
  ...
  if (redValue <= 255 && redValue >= 230) {
      window.location.href = "file:///D:/지역연계/지역연계/html/success.html";
  ...
  playerImage.src = "./character.gif";

  // 새로운 점프 중인 캐릭터 이미지 로드
  const jumpingCharacterImage = new Image();
  jumpingCharacterImage.src = "jump.png";

  // 캐릭터 이미지 로드
  const playerLeftImage = new Image();
  playerLeftImage.src = "file:///D:/지역연계/html/character_left.gif";
  
  // 캐릭터 이미지 로드
  const playerRightImage = new Image();
  playerRightImage.src = "file:///D:/지역연계/html/character_right.gif";

  // 캐릭터 이미지 로드
  const playerDownImage = new Image();
  playerDownImage.src = "file:///D:/지역연계/html/character_down.gif";
  ...
  obstacleImage.src = "obstacle.gif"; // 장애물 이미지 파일 경로로 변경

  // 코인 이미지 로드
  const coinImage = new Image();
  coinImage.src = "coin.gif"; // 코인 이미지 파일 경로로 변경

  // 도시 배경 이미지 로드
  const cityBackgroundImage = new Image();
  cityBackgroundImage.src = "game_map_test.png.jpg";

  // 새로운 미니맵 이미지 로드
  const miniMapImage = new Image();
  miniMapImage.src = "file:///D:/지역연계/html/minimap.jpg"; 
  ...
  const drawGame = () => {
    ...
    const mapStartY = canvas.height / 2 - playerY * zoomRatio + characterHeight * zoomRatio / 2; - 100;

    // 새로운 점프 중인 캐릭터 이미지 로드
    const jumpingCharacterImage = new Image();
    jumpingCharacterImage.src = "jump.gif";
